...
Re-evaluating Shader Park code in 3D mode must preserve the worker orbit-camera state when the node FBO can be reused. `updateCode()` should rebuild only the Shader Park mesh/material and keep the existing orbit state so live-coding changes do not snap the camera back to the default view.
