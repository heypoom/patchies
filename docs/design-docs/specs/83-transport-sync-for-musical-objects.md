# 83. Transport Sync for Musical Objects
...
Visual nodes (GLSL, Hydra, P5, Canvas, Three, Textmode) and `tone~` already sync to the master transport via spec 77. Audio nodes using the `clock` API (js, worker, etc.) sync via spec 80. But Strudel and Orca — the two most important standalone musical sequencers — run completely independently, drifting apart over time.
