...
- Active (green): sync enabled
  ...
  - Transport play/pause/stop/BPM/time-sig controls are **read-only** when following (same visual treatment as MIDI slave mode in spec 86)
...
**No Tone.js upgrade required.** Unlike MIDI clock sync (spec 86) which must schedule `0xF8` pulses with AudioContext timestamps, network sync sends JSON over WebRTC data channels where ±25ms precision is acceptable. All operations (`play`, `setBpm`, `seek`, BPM nudge) are available on `DefaultTransport`. `TransportSyncManager` works against the generic transport interface and does not call `ensureToneUpgraded()`. If Tone.js is already loaded for another reason, it transparently operates on the Tone.js transport instead.
...
1. Enable MIDI slave mode (spec 86)
