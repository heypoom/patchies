# 116. Render Pipeline Optimizations
...
Add a "Channels" dropdown to node settings, next to the resolution and FBO format (spec 106) dropdowns. Default: `rgba`. This is an advanced optimization — don't promote it in the UI. It's there for users who need it.

Combined with spec 106, the full FBO format options are:
...
- Cook-on-demand caching requires spec 101 (render graph diffing) as a foundation — diffing tracks what changed, caching decides whether to re-render
