# 53. Improve Csound Integration

From the [csound web audio documentation](https://waaw.csound.com/docs/index.html), we can use `CsoundNode` directly without using the higher-level `Csound` class.
...
You can create a `CsoundNode` directly via `CsoundNodeFactory`, then we can implement our own `CsoundObj` so that we can use our own audio context.

`CsoundNodeFactory.createNode(InputChannelCount, OutputChannelCount)`

## Goals

- Improve stability when using two or more csound nodes
  - The audio context crashes when using more than one csound nodes.
  - This is likely because Csound manages its own audio context which is bad. We should use our own audio context.
- Do NOT use csound's `CsoundObj` wrapper or `Csound` frontend class.
  - We need super close integration with the Web Audio API for Patchies!

## Implementation

Current implementation of Csound:

- `ui/src/lib/audio/v2/nodes/CsoundNode.ts`
- `ui/src/lib/components/nodes/CsoundNode.svelte`

Relevant docs of what we wanna use:

- <https://waaw.csound.com/docs/CsoundNode.html>
- <https://waaw.csound.com/docs/CsoundNodeFactory.html>

For reference, the public classes we do NOT want to use:

- <https://waaw.csound.com/docs/Csound.html>
- <https://waaw.csound.com/docs/CsoundObj.html>

## CsoundObj.js

You can re-create our own CsoundObj using this class. Obviously, we do NOT want to use the global `CSOUND_AUDIO_CONTEXT`.
