# Video Service V2 Migration Plan

## Overview

Refactor the 918-line `fboRenderer.ts` using a hybrid decomposition: keep a slim `VideoWorkerService` as orchestrator, extract Preview and Texture systems as helper modules, and use separate `UniformsStore` and `FFTStore` for data that survives graph rebuilds. V2 video nodes encapsulate node-specific logic.
...
| --------------------------------- | ----- | ------------------------- |
| Node factories (create\*Renderer) | ~200  | V2 Node classes           |
| Uniform/FFT management            | ~160  | V2 Node classes + Stores  |
| Message routing                   | ~30   | V2 Node classes           |
| Preview system                    | ~100  | PreviewHelper module      |
| Texture management                | ~50   | TextureStore module       |
| Render loop + frame               | ~100  | VideoWorkerService (slim) |
| Graph/FBO management              | ~80   | VideoWorkerService        |
...
```typescript
// ui/src/workers/rendering/v2/stores/UniformsStore.ts
...
```typescript
// ui/src/workers/rendering/v2/stores/FFTStore.ts
export class FFTStore {
  ...
  updateFromPayload(
    payload: AudioAnalysisPayloadWithType,
    regl: regl.Regl
  ): void;
...
### 4. TextureStore (~50 lines)

Manages external textures (images, video frames):

```typescript
// ui/src/workers/rendering/v2/stores/TextureStore.ts
...
### 6. VideoWorkerService (~200 lines)
...
2. Move uniform handling into node (reads from UniformsStore)
3. Move FFT texture handling into node (reads from FFTStore)
...
1. **No node type checks in VideoWorkerService** - use registry + node methods
2. **Stores are the source of truth** for persistent data
3. **Nodes own their render logic** - service just calls `node.render()`
4. **Async create() supported** - for nodes loading libraries
5. **Messages route through nodes** - `node.onMessage()` not service switch/match
