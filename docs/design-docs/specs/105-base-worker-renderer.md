...
export abstract class BaseWorkerRenderer<
  TConfig extends BaseRendererConfig = BaseRendererConfig,
> {
  ...
  setInteraction(mode: string, enabled: boolean) {
    /* shared */
  }
  ...
  protected resetState() {
    ...
    if (!this.settingsProxy) {
      this.settingsProxy = createWorkerSettingsProxy(
        this.config.nodeId,
  ...
  protected async executeUserCode(
    code: string,
    ...
    return this.renderer.jsRunner.executeJavaScript(
      this.config.nodeId,
      processedCode,
      {
  ...
  protected handleRuntimeError(error: unknown, wrapperOffset: number) {
    ...
    const errorInfo = parseJSError(
      error,
      countLines(this.config.code),
      wrapperOffset,
    )
    self.postMessage({
      type: 'consoleOutput',
...
export class ReglRenderer extends BaseWorkerRenderer {
  ...
  renderFrame(params: RenderParams) {
    ...
    if (this.renderer.transportTime && !this.renderer.transportTime.isPlaying)
      return
  ...
  async updateCode() {
    ...
    try {
      ...
      const userRender = await this.executeUserCode(
        codeWithWrapper,
        extraContext,
      )
