...
| Spec | Title | Summary |
|------|-------|---------|
| [105](105-multi-render-target.md) | Multi-Render-Target | Multiple video outlets per node via MRT color attachments |
| [106](106-float-fbo-format.md) | Float FBO Format | Per-node `rgba16f`/`rgba32f` for GPGPU and HDR |
| [107](107-graph-level-feedback.md) | Graph-Level Feedback | Back-edges with 1-frame delay, double-buffered FBOs |
| [108](108-visual-convenience-presets.md) | Convenience Presets | Procedural generators, post-processing, PBR, simulation presets |
| [109](109-geometry-wire-type.md) | Geometry Wire Type | Geometry handle with auto-caching inlets, attributes, instancing |
| [110](110-webgpu-render-bridge.md) | WebGPU Render Bridge | ImageBitmap + SharedArrayBuffer paths from compute to FBO pipeline |
| [111](111-shared-resource-pool.md) | Shared Resource Pool | Named cubemaps, 3D textures, LUTs accessible by any node |
| [112](112-shared-glsl-library.md) | GLSL `#include` & Effects | `#include` preprocessor (lygia, VFS, URLs), effect metadata, material system |
| [114](114-snippet-presets.md) | Snippet Presets | Cross-patch portability for GLSL/Hydra/JS snippets via preset system |
| [115](115-vfs-js-modules.md) | VFS JavaScript Modules | Import JS modules from VFS files alongside `// @lib` nodes |
| [116](116-render-pipeline-optimizations.md) | Render Pipeline Optimizations | Per-node resolution, cook-on-demand caching, channel formats, preview LOD |

## Dependency Graph

```
Independent (no deps):     105, 106, 107, 111, 112, 109 (Stage 2-3)

108 presets (some) ←── 105 (channel-split needs MRT)
108 presets (some) ←── 106 (particle-sim needs float)
108 presets (some) ←── 107 (blur needs feedback)
109 Stage 1        ←── 105 + 106 (texture-encoded geometry)
110 Bridge 2       ←── 106 (float data from compute)
```
...
1. **[112](112-shared-glsl-library.md) `#include` preprocessor** — `#include <lygia/generative/snoise>` in any shader node. Biggest quality-of-life win for live coders. No pipeline changes, no architectural risk. Includes the `glsl` tagged template literal for JS-based nodes.
2. **[107](107-graph-level-feedback.md) Feedback Loops** — Multi-node feedback chains for trails, accumulation, reaction-diffusion. Transformative for live visual workflows.

### Phase 2 — Foundations (richer textures)

3. **[105](105-multi-render-target.md) MRT** and **[106](106-float-fbo-format.md) Float FBO** — Can be built in parallel. These are the foundation that makes later specs (presets, geometry, compute bridge) much more powerful.
4. **[108](108-visual-convenience-presets.md) Presets** — Ship independent presets first (noise generators, post-processing, color grading). Add MRT/float/feedback-dependent presets as those land. Presets use `#include <lygia/...>` internally.

### Phase 3 — Expansion (new data types)

5. **[109](109-geometry-wire-type.md) Geometry** (Stage 2-3) — Geometry handle type, auto-caching inlets, `geo.*` presets (not dedicated node types). No pipeline deps.
6. **[111](111-shared-resource-pool.md) Resource Pool** — Cubemaps, LUTs, 3D textures. Important for production quality and PBR workflows.
7. **[110](110-webgpu-render-bridge.md) WebGPU Bridge** (Bridge 1) — ImageBitmap path from compute to FBO pipeline. No deps.

### Phase 4 — Full picture

8. **[109](109-geometry-wire-type.md) Geometry** (Stage 1) — Texture-encoded geometry via MRT + float FBOs. Requires 105 + 106.
9. **[110](110-webgpu-render-bridge.md) WebGPU Bridge** (Bridge 2) — SharedArrayBuffer fast path. Benefits from 106.
10. Remaining **[108](108-visual-convenience-presets.md) presets** that depend on MRT, float FBOs, or feedback.
11. **[112](112-shared-glsl-library.md) Material system** — `@slot` metadata, material presets, material preview Three.js preset. Benefits from 105 (MRT for multi-channel material output) and 111 (resource pool for environment maps).
12. **[114](114-snippet-presets.md) Snippet Presets** — Cross-patch portability for GLSL/Hydra snippets. Depends on 112.
