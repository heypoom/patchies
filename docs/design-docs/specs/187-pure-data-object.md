...
- Exposed custom message inlets appear before the dedicated control-message inlet. When no custom message inlet is exposed, the control inlet is the only message inlet. Reordering changes only handle positions: the control inlet keeps its stable ID and existing custom inlet IDs remain unchanged.
...
- An unmounted empty node is immediately editable. Its first edit persists an inline patch in `sourceCode`.
- A `patch://` source is mounted read-write. Edits from the node editor or its Objects VFS projection write directly to the embedded patch file, keep `sourceCode` null, and reload the runtime from that path.
- A `user://` or HTTP(S) source is mounted read-only. It cannot be edited from the node editor or Objects VFS projection until detached.
- Every mounted source offers an explicit Detach action. Detaching copies the currently resolved source into `sourceCode`, clears `vfsPath` and `sourceUrl`, clears the visible path field, and changes the node label to `Inline patch`. Detaching from `patch://` is optional; detaching from `user://` or HTTP(S) is required before editing.
- Loading `{ type: 'load', code }` creates an inline patch and clears any mounted source. Loading a new VFS path or HTTP(S) URL clears `sourceCode` to `null`, so the new external source becomes authoritative. Source selection prefers `sourceCode`, then `sourceUrl`, then `vfsPath`. Inline edits, detach, and embedded `patch://` writes participate in undo and redo.
