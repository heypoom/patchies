# 112. GLSL `#include` Preprocessor & Shader Effect Format
...
- Render nodes (spec 109) can auto-map slots to material properties
...
- Has settings for preview shape (sphere, cube, plane, custom geometry via spec 109)
- Has settings for lighting environment (from resource pool via spec 111, or built-in presets: studio, outdoor, dramatic)
...
- Materials benefit from spec 105 (MRT) for multi-channel output and spec 111 (resource pool) for environment maps
