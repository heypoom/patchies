...
Encode geometry as float textures — position, normal, UV maps. No new wire type needed. A GLSL or SwissGL node outputs float32 textures (spec 106) via MRT (spec 105), and a downstream Three.js node reads them as vertex data.
...
Users fork the preset for more control: swap in a PBR material with `@slot` textures (spec 112), add orbit controls, use an environment cubemap from the resource pool (spec 111), accept multiple geometry inlets for scene composition. It's just a Three.js node.
...
- Stage 1 requires: spec 105 (MRT), spec 106 (float FBOs)
