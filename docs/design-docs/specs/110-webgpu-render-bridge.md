# 110. WebGPU ↔ Render Pipeline Bridge
...
**When to use**: Large data transfers where ImageBitmap's RGBA8 format is limiting (float data, non-image buffers). This pairs with spec 106 (float FBOs) — compute produces float data, render worker uploads to rgba32f texture.
...
- Float texture output benefits from spec 106 (float FBOs)
