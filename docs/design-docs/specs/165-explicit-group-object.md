...
- Group locking stores `node.data.locked`, disables movement of the group frame itself by setting the node-level `draggable` state, and excludes the group frame from drag-selection results.
