...
`presets` remains the flat top-level list. `presetFolders` is for large built-in archives that need
folder organization inside the built-in preset tree. The enable/disable store flattens both fields
when deciding which preset names are enabled.
...
- Generated by fetching `https://greggman.github.io/html5bytebeat/editor/songs.json`
- Replaces the hand-copied `Bytebeat Demos` entries, since those came from the same archive
- Uses preset subfolders based on the source `size` field, matching the source site's browsing
  model:
  - `Tiny (<80 chars)`
  - `Small (80-199 chars)`
  - `Medium (200-999 chars)`
  - `Long (1000+ chars)`
- Each link's `bb` hash param is hex-encoded LZMA-Alone source code. Generation decodes it into
  static `bytebeat~` presets so the app does not need browser-side archive decoding.
