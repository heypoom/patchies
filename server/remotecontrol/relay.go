...
func validateCommit(request CommitRequest) error {
	...
	if len(request.Changes) == 0 {
		return errors.New("applied commit requires at least one entry change")
	}
	for _, change := range request.Changes {
		if change.Path == "" || len(change.Entry) == 0 || !json.Valid(change.Entry) {
			return errors.New("commit contains an invalid entry change")
		}
	}
...
func cloneChanges(changes []EntryChange) []EntryChange {
	result := make([]EntryChange, len(changes))
	for index, change := range changes {
		result[index] = EntryChange{Path: change.Path, Entry: append(json.RawMessage(nil), change.Entry...)}
	}
