...
### `wgpu.compute`: WebGPU compute shaders

- Run parallel computations on the GPU using [WGSL](https://www.w3.org/TR/WGSL/) (WebGPU Shading Language).
- Great for data-parallel operations like noise generation, matrix math, and array processing.
...
2. Input buffers use `var<storage, read>`, output buffers use `var<storage, read_write>`
...
- `{type: 'setCode', code: '...'}` - update the shader code
- `{type: 'run'}` - compile the shader

```wgsl
// Example: Double all input values
