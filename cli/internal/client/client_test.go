...
func TestAttachDoesNotFollowRedirects(t *testing.T) {
	redirected := false
	...
	_, err := New(protocol.Connection{InstanceURL: origin.URL, SessionID: "session", Secret: "secret"}).Attach(t.Context())
	if err == nil {
		t.Fatal("redirect accepted")
	}
	if redirected {
		t.Fatal("bearer request followed redirect")
	}
}
