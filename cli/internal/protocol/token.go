...
func ParseConnection(value string) (Connection, error) {
	...
	host := strings.ToLower(instanceURL.Hostname())
	ip := net.ParseIP(host)
	if instanceURL.Scheme == "http" && host != "localhost" && (ip == nil || !ip.IsLoopback()) {
		return Connection{}, errors.New("remote control requires HTTPS except on localhost or a loopback IP")
	}
