...
import { BiquadPortSchema } from '$objects/biquad~/native-dsp/schemas/biquad.schema';
...
defineDSP({
  ...
  process(state, inputs, outputs, _send, parameters) {
    ...
    for (let i = 0; i < len; i++) {
      const x0 = input[i];
      const ff1 = p(ff1Param, i);
      const ff2 = p(ff2Param, i);
      const ff3 = p(ff3Param, i);
      const fb1 = p(fb1Param, i);
      const fb2 = p(fb2Param, i);
