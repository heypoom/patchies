...
import { VfsChangeTracker } from './vfs-change-tracker';
...
export class RemoteControlSyncCoordinator {
  ...
  notifyPatchChanged(patchId = this.options.patchId()): void {
    if (!this.credentials) return;

    if (patchId !== this.sessionPatchId) {
      this.schedulePatchTransition(patchId);
      return;
    }
    ...
    await this.request(
      `/api/remote-control/sessions/${this.requireCredentials().sessionId}/snapshot`,
      {
        ...
        body: {
          ...
          representation: {
            format: MOUNT_FORMAT,
            patchId: this.requireSessionPatchId(),
            entries: [...entries, ...mountReferences]
          }
        }
      }
    );
    this.changeTracker.reset(entries);
  }
  ...
  private schedulePatchTransition(patchId: string): void {
    if (this.syncTimer) {
      clearTimeout(this.syncTimer);
      this.syncTimer = null;
    }
  ...
  private watchFiles(): void {
    this.unsubscribeFiles?.();
    this.unsubscribeFiles = this.options.filesystem.entries$.subscribe(() =>
