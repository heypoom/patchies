import { Controller, type MachineStatus } from 'machine';
...
export class AssemblySystem {
	...
	/**
	 * Consume and return events generated by a machine
	 */
	consumeMachineEvents(machineId: number): any[] {
		try {
			return this.controller.consume_machine_side_effects(machineId);
		} catch (error) {
			console.error(`Failed to consume events from machine ${machineId}:`, error);
			return [];
		}
	}
	 ...
	 */
	sendMessage(machineId: number, message: any): boolean {
		try {
