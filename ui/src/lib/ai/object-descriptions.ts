...
\`\`\`json
{
  "nodes": [
    ...
    { 
      "type": "tone~", 
      "data": { 
        "code": "setPortCount(1);\\nsetTitle('808 Kick');\\n\\nconst kick = new Tone.MembraneSynth({ pitchDecay: 0.05, octaves: 10, oscillator: { type: 'sine' } }).connect(outputNode);\\n\\nrecv(m => { if (m) kick.triggerAttackRelease('C2', '8n'); });\\n\\nreturn { cleanup: () => kick.dispose() };" 
      },
      "position": { "x": 0, "y": 150 }
    },
...
\`\`\`
Note: button → tone~ uses targetHandle: "message-in-0" (NOT "message-in")`;
