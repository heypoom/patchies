...
function generateManifest(root: string): void {
  ...
  const PREFIX = 'export const HELP_PATCHES_AVAILABLE: ReadonlySet<string> = new Set(';
  const SUFFIX = ');';
  const items = objectTypes.map((t) => `'${t}'`);
  const singleLine = `${PREFIX}[${items.join(', ')}]${SUFFIX}`;

  // Use single-line if it fits within printWidth (100), otherwise multi-line
  const setLine =
